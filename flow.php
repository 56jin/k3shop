...
if ($_REQUEST['step'] == 'add_to_cart')
{
    ...
    $goods = $json->decode($_POST['goods']);
		...
		/* 代码增加_判断是否限购_start By www.ecshop120.com */
	$time_xg_now=gmtime();
	...
	if ( $row_xg['buymax'] >0 && $row_xg['buymax_start_date'] < $time_xg_now  && $row_xg['buymax_end_date'] > $time_xg_now  )
	{
		if ( !$_SESSION['user_id'] )
		{
			$result['error']  = 999;
		...
		else
		{
			...
			if ( $num_total > intval($row_xg['buymax']) )
			{
				...
				$result['message'] ="注意：\n\r此商品促销期间每人限购 ". $row_xg['buymax'] . " 件\n\r";
				if ($num_cart_old_2)
				{
					$result['message'] .="您之前已经成功购买过 $num_cart_old_2 件！\n\r";
				}
				if ($num_cart_old_1)
				{
	...
	/* 代码增加_判断是否限购_end  By www.ecshop120.com */
...
elseif ($_REQUEST['step'] == 'fittings_to_cart')
{
    ...
    $goods_arr = json_str_iconv($_POST['goods_arr']);
	$goods = explode(",",$goods_arr);
	$parent_id = json_str_iconv($_POST['parent_id']);
	
	// 更新：添加到购物车
	foreach($goods as $val){

        if (addto_cart($val,1,$spec = array()))
        {
            if ($_CFG['cart_confirm'] > 2)
            {
                $result['message'] = '';
            ...
            //$result['content'] = insert_cart_data();
            $result['one_step_buy'] = $_CFG['one_step_buy'];
        }
        else
        {
