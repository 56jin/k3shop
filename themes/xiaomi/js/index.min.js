...
XIAOMI.namespace("app.formatter, app.recharge"),
XIAOMI.app.formatter = {
	...
	getCaret: function(a) {
		var b = $(a)[0];
		if ("number" == typeof b.selectionStart) return {
			begin: b.selectionStart,
			end: b.selectionEnd
		};
		...
		if (c && c.parentElement() === b) {
			...
			e = b.createTextRange(),
			f = b.value.length;
			return d.moveToBookmark(c.getBookmark()),
			e.collapse(!1),
			d.compareEndPoints("StartToEnd", e) > -1 ? {
				begin: f,
				end: f
			}: {
				begin: -d.moveStart("character", -f),
				end: -d.moveEnd("character", -f)
			}
		}
		return {
			begin: 0,
	...
	formatString: function(a, b) {
		var c, d, e, f, g = this;
		c = a.val(),
		e = g.getTrimString(c, b.separatorSymbol),
		f = b.separatorSymbol.length;
		for (var h = 0; h < b.formatPattern.length - 1; h++) d = g.getSumTotal(b.formatPattern, h),
		e.length >= d + h * f && (e = [e.slice(0, d + f * h), e.slice(d + f * h)].join(b.separatorSymbol));
		a.val(e)
	},
	init: function(a, b) {
		...
		d = $.extend({},
		c, b),
		f = d.separatorSymbol.length,
		e = (d.formatPattern.length - 1) * f,
		g = function(a) {
			var b = "undefined" != typeof navigator ? navigator.userAgent: null,
			c = /iphone/i.test(b);
		...
		k.on({
			...
			keyup: function(a) {
				var b = $(this).val();
				b.length > d.stringLength + e && ($(this).val(b.substring(0, d.stringLength + e)), j.setCaret($(this), l.pos)),
				g(a.which) || h(a.which) || i(a) || (j.formatString($(this), d), j.setCaret($(this), l.pos))
			},
			paste: function() {
				var a = $(this).val();
				a.length > d.stringLength + e && $(this).val(a.substring(0, d.stringLength + e)),
				j.formatString($(this), d)
			}
		})
	}
},
XIAOMI.app.recharge = {
	init: function(a) {
		...
		function c(a) {
			return XIAOMI.app.formatter.getTrimString(a, "-")
		}
		function d(a) {
		...
		function g(a, b) {
			window.location.href = XIAOMI.GLOBAL_CONFIG.orderSite + "/recharge/submit?mobile=" + a + "&pervalue=" + b
		}
		function h(a, b) {
			var e = XIAOMI.GLOBAL_CONFIG.orderSite + "/recharge/checkout";
			m.val("数据加载中…"),
			$.ajax({
				type: "get",
				url: e,
				...
				success: function(a) {
					if (a.boolean) {
						...
						var b = j.html();
						b !== a.recharge_info && j.html(a.recharge_info),
						d(!1),
						a.preferential.boolean && j.html(a.preferential.message),
						m.on("click",
						function(a) {
	...
	function($, window, document) {
		...
		Plugin.prototype.init = function() {
			...
			}), $.each($(".xm-slider-control", $element).children(),
			function(i) {
				var $slide;
				return $slide = $(this),
				$slide.attr("xm-slider-index", i)
			}), this.data.touch && ($(".xm-slider-control", $element).on("touchstart",
			function(e) {
				return _this._touchstart(e)
			}), $(".xm-slider-control", $element).on("touchmove",
			function(e) {
				return _this._touchmove(e)
			}), $(".xm-slider-control", $element).on("touchend",
			function(e) {
				return _this._touchend(e)
			})), $element.fadeIn(0), $element.find("img").each(window.devicePixelRatio < 1.5 ?
			function() {
			...
			}), this.update(), this.data.touch && this._setuptouch(), $(".xm-slider-control", $element).children(":eq(" + this.data.current + ")").eq(0).fadeIn(0,
			function() {
				return $(this).css({
					zIndex: 10
				})
			...
			}).appendTo($element), $.each(new Array(this.data.total),
			function(i) {
				...
				paginationLink.click(function(e) {
					return e.preventDefault(),
					_this.stop(!0),
					_this.goto(1 * $(e.currentTarget).attr("data-xm-slider-item") + 1)
				})
			})), $(window).bind("resize",
			function() {
				return _this.update()
		...
		Plugin.prototype._setActive = function(number) {
			...
			$(".xm-slider-pagination li:eq(" + current + ") a", $element).addClass("active")
		},
		Plugin.prototype.update = function() {
			...
			width <= this.options.responsiveWidth && $element.find("img").each(window.devicePixelRatio < 1.5 ?
			function() {
		...
		Plugin.prototype._setuptouch = function() {
			...
			return $element = $(this.element),
			this.data = $.data(this),
			slidesControl = $(".xm-slider-control", $element),
		...
		Plugin.prototype._touchend = function(e) {
			...
			touches = e.originalEvent.touches[0],
			slidesControl = $(".xm-slider-control", $element),
			slidesControl.position().left > .5 * this.options.width || slidesControl.position().left > .1 * this.options.width && Number(new Date) - this.data.touchtimer < 250 ? ($.data(this, "direction", "previous"), this._slide()) : slidesControl.position().left < -(.5 * this.options.width) || slidesControl.position().left < -(.1 * this.options.width) && Number(new Date) - this.data.touchtimer < 250 ? ($.data(this, "direction", "next"), this._slide()) : (prefix = this.data.vendorPrefix, transform = prefix + "Transform", duration = prefix + "TransitionDuration", timing = prefix + "TransitionTimingFunction", slidesControl[0].style[transform] = "translateX(0px)", slidesControl[0].style[duration] = .85 * this.options.effect.slide.speed + "ms"),
		...
		Plugin.prototype.play = function(next) {
			...
			function() {
				return $(".xm-slider-navigation", $element).show(),
				_this.stop()
			}), slidesContainer.bind("mouseleave",
			function() {
				return $(".xm-slider-navigation", $element).hide(),
				_this.play()
		...
		Plugin.prototype.stop = function(clicked) {
			...
			this.options.play.pauseOnHover && clicked && $(".xm-slider-container", $element).unbind(),
			$.data(this, "playInterval", null),
			$.data(this, "playing", !1),
			$(".xm-slider-play", $element).removeClass("xm-slider-playing"),
			this.options.play.swap ? ($(".xm-slider-stop", $element).hide(), $(".xm-slider-play", $element).show()) : void 0
		},
		Plugin.prototype._slide = function(number) {
			var $element, currentSlide, direction, duration, next, prefix, slidesControl, timing, transform, value, _this = this;
			return $element = $(this.element),
			this.data = $.data(this),
			this.data.animating || number === this.data.current + 1 ? void 0 : ($.data(this, "animating", !0), currentSlide = this.data.current, number > -1 ? (number -= 1, value = number > currentSlide ? 1 : -1, direction = number > currentSlide ? -this.options.width: this.options.width, next = number) : (value = "next" === this.data.direction ? 1 : -1, direction = "next" === this.data.direction ? -this.options.width: this.options.width, next = currentSlide + value), -1 === next && (next = this.data.total - 1), next === this.data.total && (next = 0), this._setActive(next), slidesControl = $(".xm-slider-control", $element), number > -1 && slidesControl.children(":not(:eq(" + currentSlide + "))").css({
				display: "none",
				left: 0,
				zIndex: 0
			}), slidesControl.children(":eq(" + next + ")").css({
		...
		Plugin.prototype._fade = function(number) {
			...
			return $element = $(this.element),
			this.data = $.data(this),
			this.data.animating || number === this.data.current + 1 ? void 0 : ($.data(this, "animating", !0), currentSlide = this.data.current, number ? (number -= 1, value = number > currentSlide ? 1 : -1, next = number) : (value = "next" === this.data.direction ? 1 : -1, next = currentSlide + value), -1 === next && (next = this.data.total - 1), next === this.data.total && (next = 0), this._setActive(next), slidesControl = $(".xm-slider-control", $element), slidesControl.children(":eq(" + next + ")").css({
				display: "none",
				left: 0,
				zIndex: 10
...
XIAOMI.namespace("mhome"),
XIAOMI.mhome = {
	commentedGoods: function() {
		var cidStr, $commentedGoods = $(".J_commentedGoods");
		if (0 === isCommentOpen) return ! 1;
		for (var i = 0,
		lens = $commentedGoods.length; lens > i; i += 1) cidStr = 0 === i ? $commentedGoods.eq(i).attr("data-cid") : cidStr + "-" + $commentedGoods.eq(i).attr("data-cid");
		$.ajax({
			type: "GET",
			url: XIAOMI.GLOBAL_CONFIG.wwwSite + "/comment/total/id/" + cidStr,
			dataType: "jsonp",
			...
			success: function(data) {
				if (0 === data.errorno) for (var i in data.totalArr) if (data.totalArr.hasOwnProperty(i)) {
					...
					if ($goods.length) {
						...
						0 !== data.totalArr[i].total && (html += '<span class="sep">|</span>' + data.totalArr[i].total + "人评价"),
						$goods.find(".item-meta").html(html)
					}
				}
			}
		})
...
jQuery(function($) {
	...
	}), XIAOMI.app.carousel.init($(".J_starGoodsCarousel"), {
		itemSelector: ".xm-star-goods-item"
	}), XIAOMI.app.recharge.init({
		elmTel: $("#rechargeTel"),
		elmTips: $("#rechargeTips"),
		elmAmount: $("#rechargeAmount"),
		elmPrice: $("#rechargePrice"),
		elmSubmit: $("#rechargeBtn")
	}), XIAOMI.app.formatter.init($("#rechargeTel"), null), XIAOMI.app.selector.init($("#rechargeAmount"), {}), XIAOMI.mhome.commentedGoods(), XIAOMI.mhome.secKillGoods(), XIAOMI.app.lazyLoad(), XIAOMI.app.analytics([".xm-slider-container", ".home-hd-show", ".xm-star-goods-list", ".home-new-goods .brick-list", ".home-hot-goods .brick-list", ".board-section", ".home-commented-goods .xm-commented-goods-list", ".home-saleoff-goods-list", ".home-duokan-goods .xm-commented-goods-list", ".home-miui-goods .xm-commented-goods-list", ".news-section", ".channel-list", ".list-service"], {
		position: ["A1", "A2", "B1", "B2", "C1", "C2", "D1", "D2", "E1", "E2", "NEWS", "specia", "service"],
		mstPage: "HP",
		mstArea: "AD",
		mstPosition: ["A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K", "", ""],
		isMst: !0
	}), XIAOMI.app.analytics([".site-topbar"], {
		position: ["A"],
		url: !1
	}), -1 === window.location.href.indexOf("index.php")) {
		var userLanguage = XIAOMI.app.cookie("userLanguage"),
		browserLanguage = window.navigator.language || window.navigator.userLanguage;
		if (browserLanguage = browserLanguage.toLowerCase(), "zh-tw" === userLanguage) window.location.href = "http://www.mi.com/tw/";
		else if ("zh-hk" === userLanguage) window.location.href = "http://www.mi.com/hk/";
		else if ("zh-sg" === userLanguage) window.location.href = "http://www.mi.com/sg/";
		else if ("us-en" === userLanguage) window.location.href = "http://www.mi.com/en/";
		else if ("zh-cn" !== userLanguage) if ("zh-tw" === browserLanguage) XIAOMI.app.cookie("browserLanguage", "zh-tw", {
			expires: 365,
			path: "/"
		}),
		window.location.href = "/misites/redirect/";
		else if ("zh-hk" === browserLanguage) XIAOMI.app.cookie("browserLanguage", "zh-hk", {
			expires: 365,
			path: "/"
		}),
		window.location.href = "/misites/redirect/";
		else if ("zh-sg" === browserLanguage) XIAOMI.app.cookie("browserLanguage", "zh-sg", {
			expires: 365,
			path: "/"
		}),
		window.location.href = "/misites/redirect/";
		else if ("zh-cn" !== browserLanguage) $.ajax({
			dataType: "JSONP",
			url: "http://region.www.xiaomi.com/x_s_n_m_z_h_s_region.json",
			jsonpCallback: "redirectcallback",
			success: function(data) {
				"SG" === data.code ? (XIAOMI.app.cookie("browserLanguage", "zh-sg", {
					expires: 365,
					path: "/"
				}), window.location.href = "/misites/redirect/") : "CN" !== data.code && "HK" !== data.code && "TW" !== data.code && (XIAOMI.app.cookie("browserLanguage", "us-en", {
					expires: 365,
					path: "/"
			...
			if ("mapp" === XIAOMI.app.cookie("orgin")) return void XIAOMI.app.cookie("orgin", null, {
				path: "/",
				domain: ".mi.com"
			}); (ua.indexOf("Android") > -1 || ua.indexOf("iPhone") > -1 || ua.indexOf("iPod") > -1 || ua.indexOf("Symbian") > -1) && (location.href = "http://m.mi.com/home/index.html")
		}
	}
});