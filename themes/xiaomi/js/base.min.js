...
XIAOMI.app.setLoginInfo = {
	...
	init: function(globalConfig) {
		...
		this.uuidCookie(),
		this.miid && 0 != this.miid ? (this.data.userName = this.miid ? XIAOMI.app.cookie("XM_" + this.miid + "_UN") : "", this.data.goodsNum = XIAOMI.app.cookie("xm_user_www_num"), this.data.goodsNum = null == this.data.goodsNum ? 0 : this.data.goodsNum, null == this.data.userName || "" === this.data.userName ? _this.createScript(this.miid) : this.updateDom()) : $.ajax({
			dataType: "JSONP",
			...
			success: function(data) {
				if (0 != data.userId && null != data.userId) {
					...
					XIAOMI.app.cookie("userId", data.userId, option),
					_this.createScript(data.userId)
				}
			}
		})
	},
	createScript: function(uid) {
		...
		script.src = "https://account.xiaomi.com/pass/userInfoJsonP?userId=" + uid + "&callback=XIAOMI.app.setLoginInfo.getAccountInfo",
	...
	updateDom: function() {
		var displayName = "";
		displayName = this.data.userName.length > 7 ? this.data.userName.substr(0, 7) + "…": this.data.userName;
		var timeoutUserMenu, html = '欢迎您 <b class="user-name">' + displayName + '<span class="arrow"></span></b><span class="sep">|</span><a href="' + this.config.orderSite + '/user/order">我的订单</a><span class="sep">|</span><a href="http://account.xiaomi.com/" target="_blank">我的小米账户</a>',
	...
	getAccountInfo: function(data) {
		if (data.userId) {
			...
			XIAOMI.app.cookie("XM_" + this.miid + "_UN", this.data.userName, option),
			this.updateDom()
		}
	},
	uuidCookie: function() {
...
XIAOMI.app.setRecommend = function(options) {
	var option = {
